...
void WlanHostedNetworkHelper::Start()
{
	...
	while (GetPrivateProfileStringW(L"AP", L"Pass", NULL, buffer, length, file) == length - 1)
	{
		buffer = (LPWSTR)_recalloc(buffer, (length *= 2), sizeof(wchar_t));
	}
	// buffer is Password
	hr = hstrPassphrase.Set(buffer);
	free(buffer);
	if (FAILED(hr))
	{
